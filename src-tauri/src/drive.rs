...
pub fn backup_data() -> Result<String, String> {
    ...
    // 1. Check if file exists to get ID
    let list_url = "https://www.googleapis.com/drive/v3/files?q=name='timigs_backup.db' and trashed=false";
    ...
    let files = body["files"].as_array().ok_or("Failed to parse drive response")?;
    
    let file_id = if let Some(file) = files.first() {
        file["id"].as_str().map(|s| s.to_string())
    } else {
        None
    };
    ...
    if let Some(id) = file_id {
        ...
        let update_url = format!("https://www.googleapis.com/upload/drive/v3/files/{}?uploadType=media", id);
        ...
        Ok(format!("Backup updated (ID: {})", id))
    } else {
        ...
        let form = reqwest::blocking::multipart::Form::new()
            .part("metadata", reqwest::blocking::multipart::Part::text(metadata).mime_str("application/json").unwrap())
            .part("file", reqwest::blocking::multipart::Part::bytes(content).mime_str("application/x-sqlite3").unwrap());
...
pub fn restore_data() -> Result<String, String> {
    ...
    match fs::rename(&temp_path, &db_path) {
        ...
        Err(_) => {
            ...
            Err("Cannot replace database file while in use. Please close app and replace manually.".to_string())
        }
    }
}
