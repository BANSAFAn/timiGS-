...
if (fs.existsSync(jsonDir)) {
  ...
  for (const file of files) {
    ...
    try {
      ...
      if (data.platforms && data.platforms.platforms && data.platforms.platforms.android !== undefined) {
        delete data.platforms.platforms.android;
        modified = true;
      }
...
if (fs.existsSync(tsPath)) {
  ...
  let newContent = tsContent.replace(/android:\s*'[^']*',\s*android_desc:\s*'[^']*',\s*android_alpha_badge:\s*'[^']*',\s*android_alpha_warning:\s*'[^']*',/g, '');
